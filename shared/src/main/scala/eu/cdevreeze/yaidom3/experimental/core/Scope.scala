 ...
 *   Chris de Vreeze
 */
final case class Scope private (prefixedNamespaceMap: ListMap[String, String], defaultNamespaceOption: Option[String]):
  assert(!prefixedNamespaceMap.contains(Declarations.XmlPrefix))
   ...
   */
  def filter(p: (Option[String], String) => Boolean): Scope =
    Scope(
   ...
   */
  def filterOptPrefixes(p: Option[String] => Boolean): Scope =
    Scope(
  ...
  def prefixes: Set[String] = prefixedNamespaceMap.keySet

  /**
   * Returns the prefixes, wrapped in an Option, and None as well if the default namespace exists.
   */
  def optPrefixes: Set[Option[String]] = prefixes.map(Option(_)).concat(defaultNamespaceOption.map(_ => None).toSet)

  def namespaces: Set[String] = prefixedNamespaceMap.values.toSet.concat(defaultNamespaceOption.toSet)

  /**
   * Resolves the passed QName, if possible, wrapping the result in an Option.
   */
  def resolveOption(qname: QName): Option[EName] =
    qname match
      case QName.UnprefixedName(localPart) if defaultNamespaceOption.isEmpty =>
        Some(EName.of(localPart))
      case QName.UnprefixedName(localPart) =>
        Some(EName.of(defaultNamespaceOption, localPart))
      case QName.PrefixedName(prefix, localPart) =>
        prefix match
          case Declarations.XmlPrefix => Some(EName.of(Declarations.XmlNamespace, localPart))
          case _ => prefixedNamespaceMap.get(prefix).map(ns => EName.of(ns, localPart))
  end resolveOption

  /**
   * Returns the equivalent of `resolveOption(qname).get`.
   */
  def resolve(qname: QName): EName =
    resolveOption(qname)
  ...
  def resolve(declarations: Declarations): Scope =
    if declarations.isEmpty then Scope.this // fast for this usual case
      ...
      val newDefaultNamespaceOption: Option[String] =
      ...
      val prefixesToUndeclare: Set[String] = declarations.retainingUndeclarations.prefixedDeclarationMap.keySet
   ...
   */
  def unsafeAppendCompatibly(prefix: String, namespace: String): Scope =
    require(
    ...
    require(prefix != Declarations.XmlPrefix, s"Cannot add prefix '${Declarations.XmlPrefix}'")
   ...
   */
  def unsafeAppendCompatibly(prefixNamespaceMap: ListMap[String, String]): Scope =
    require(!(prefixNamespaceMap.keySet.contains(Declarations.XmlPrefix)), s"Cannot add prefix '${Declarations.XmlPrefix}'")
...
object Scope:
  ...
  def from(prefixedNamespaceMap: ListMap[String, String], defaultNamespaceOption: Option[String]): Scope =
    apply(prefixedNamespaceMap.filterNot(_._1 == Declarations.XmlPrefix), defaultNamespaceOption)

  def from(prefixOptionNamespaceMap: ListMap[Option[String], String]): Scope =
    from(
  ...
  def areCompatible(scope1: Scope, scope2: Scope): Boolean =
    scope1.defaultNamespaceOption == scope2.defaultNamespaceOption && {
      val commonPrefixes: Set[String] = scope1.prefixedNamespaceMap.keySet.intersect(scope2.prefixedNamespaceMap.keySet)
  ...
  def commonPrefixedScope(scopes: Seq[Scope]): Scope =
    ...
    val prefixNamespaceMap: Map[String, Seq[String]] =
      distinctScopes.flatMap(_.prefixedNamespaceMap).distinct.groupMap(_._1)(_._2)
    val commonPrefixMap: ListMap[String, String] = prefixNamespaceMap.toSeq
      .collect {
